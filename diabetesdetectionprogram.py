print("\n= = = = = = = = = = = = = = = = = = = = = = = = = = = = = =")
print("  <- - - - - - -DIABETES DETECTION PROGRAM- - - - - - ->")
print("= = = = = = = = = = = = = = = = = = = = = = = = = = = = = =\n")
...
print("\n   < LOADING DATA >",end="                             ")
try:
    data = pd.read_csv("pima_indians_diabetes_dataset.csv")
    print("[✓]\n")
except FileNotFoundError:
    print("\n ERROR: 'pima_indians_diabetes_dataset.csv' not found;\n\n Download the Pima Indians Diabetes Dataset:\n https://www.kaggle.com/datasets/uciml/pima-indians-diabetes-database\n")
    exit()

print("\n   < MISSING DATA HANDLING >",end="                    ")
illogical_features = ["Glucose","BloodPressure","SkinThickness","Insulin","BMI"]
...
print(f" • Total Actual Positive Cases (Diabetic): {total_actual_pos}\n")
print(f" • Total Actual Negative Cases (Non-Diabetic): {total_actual_neg}\n")
print("\n- - - - - - - - - - - - - - - - - - - - - - - - - - - - - -")
print("                   - ERROR ANALYSIS -")
print("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n")
print(f" • Recall (Sensitivity): {recall:.4f}")
print(f" * Model caught {recall*100:.2f} % of all actual diabetic patients")
print(f" * Missed Cases: {FN} [ !! CRITICAL ERROR !! ]\n")
print(f" • Precision: {precision:.4f}")
print(f" * Model predicted positive correctly {precision*100:.2f} % of the time")
print(f" * Falsely Predicted Diabetic: {FP}\n")
print(f" • Specificity: {specificity:.4f}")
print(f" * Model identified {specificity*100:.2f} % of all non-diabetic patients\n")
print("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n")